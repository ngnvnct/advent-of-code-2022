...
def build(cave_walls, matrix):
    for wall in cave_walls.values():    # building the cave in the numpy matrix
        for i in range(1, len(wall)):
            x_before, y_before = wall[i-1][0], wall[i-1][1]
            curr_x, curr_y = wall[i][0], wall[i][1]
...
def main():
    ...
    matrix = np.zeros((max_y+3, 1000))   # "empty" numpy array of the cave. Walls will be filled in as "#". 
    matrix = build(cave_walls, matrix)
